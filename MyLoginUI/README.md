...
As an IT administrator, you have complete control of authentication by funneling it through one point. You don't have to worry about multiple implementations that could potentially have security flaws.

As a C-level executive in charge of my companies mobile strategy, you have complete control of the branding used in applications. 
...
Details:
ReusableLoginPage exposes button presses through [virtual methods](https://github.com/michael-watson/MyExpenses/blob/master/MyLoginUI/MyLoginUI/ReusableLoginPage.cs#L186) that enables a developer to override these functions to provide whatever they needed. If your organization doesn't have a single authentication point (maybe there are multiple depending on region), you can still use all of this and the virtual methods enable your developers to plug in whatever they need. The virtual methods are exposed from the [button clicked event handlers](https://github.com/michael-watson/MyExpenses/blob/master/MyLoginUI/MyLoginUI/ReusableLoginPage.cs#L104). 
